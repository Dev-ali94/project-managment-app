...
const TaskDetails = () => {
  ...
  // Fetch task and project details from local Redux workspace
  ...
  const fetchComments = async () => {
    if (!taskId) return;
    try {
      const { data } = await api.get(`/api/comment/${taskId}`, {
        headers: {
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      // Add the newly created comment to state
  ...
  // Initial fetch of task/project
  useEffect(() => {
    fetchTaskDetails();
  }, [taskId]);

  // Fetch comments when task/project is loaded
  useEffect(() => {
    if (taskId && task) {
      fetchComments();
      const interval = setInterval(fetchComments, 10000); // refresh every 10s
  ...
  if (!task)
    return (
      <div className="text-red-500 px-4 py-6">Task not found.</div>
    );
